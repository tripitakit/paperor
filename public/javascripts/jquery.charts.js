(function($)
{
	...
	$.fn.charts = function (settings) {
		...
		function GetHorizontalBarsOutput(groupArray, labelArray, valueArray, totalValue, largestValue) {
			...
			for (var i = 0; i < valueArray.length; i++) {
				...
				if (config.showlabels) {
					displayLabel = "<a href='#" + labelArray[i] + "'>"+  labelArray[i] +  "<br>";
				}
				
				output += "<li>" + displayLabel + "<span class=\"" + config.classmodifier + "bar\" style=\"display: block; width: 0%; background-color: " + config.chartbgcolours[colourIndex] + "; color: " + config.chartfgcolours[colourIndex] + "; padding: " + config.chartpadding + "px 0; text-align: right;\" rel=\"" + barWidth + "\" >" + valueArray[i] + "&nbsp;</span></li>";
		...
		function GetWaterfallOutput(labelArray, valueArray, totalValue, largestValue) {
			var output = "";
			var colourIndex = 0;
			var leftShim = 0;
			...
			for (var i = 0; i < valueArray.length; i++) {
				...
				// Column
				output += "<div class=\"" + config.classmodifier + "bar\" style=\"position: absolute; bottom: " + bottomPosition + "%; left: " + leftShim + "%; display: block; height: 0%; background-color: " + config.chartbgcolours[colourIndex] + "; color: " + config.chartfgcolours[colourIndex] + "; width: " + widthAdjustment + "%; text-align: center;\" rel=\"" + barHeight + "\" title=\"" + labelArray[i] + " - " + valueArray[i] + " (" + percent + "%)" + "\">" + valueArray[i] + displayLabel + "</div>"
			...
			switch (config.charttype) {
				...
				case 'waterfall':
					...
					switch (config.direction) {
						...
						case 'vertical':
							// Waterfall chart
							output += GetWaterfallOutput(labelArray, valueArray, totalValue, largestValue);
							break;
					}
					break;
